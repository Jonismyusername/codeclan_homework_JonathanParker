...
SELECT
COUNT(*) AS missing_local_account_iban
FROM pay_details
WHERE local_account_no IS NULL AND iban IS NULL;
...
*/
SELECT
COUNT(*) AS employees_first_name_f
FROM employees
...
*/
SELECT
COUNT(*) AS France_Germany_pension_enrolled
FROM employees
...
*/
SELECT
MAX(salary) AS max_engineering_salary_full_time
FROM employees
...
/*
Question 16.
The corporation wants to make name badges for a forthcoming conference. Return a column badge_label showing employees’ 
first_name and last_name joined together with their department in the following style: ‘Bob Smith - Legal’. Restrict output 
...
to celebrate long-standing colleagues, in the following style ‘Bob Smith - Legal (joined 1998)’. Further restrict output 
...
/*
Question 18.
Return the first_name, last_name and salary of all employees together with a new column called salary_class with a value 
'low' where salary is less than 40,000 and value 'high' where salary is greater than or equal to 40,000. 
*/
SELECT
	first_name,
	last_name,
